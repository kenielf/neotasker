...
Abaixo, segue uma lista de alguns componentes possíveis com o AWT:
\begin{itemize}
	\item \textbf{Label (Rótulo):} Representa um componente que exibe um texto não editável; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{Button (Botão):} Representa um componente que, quando clicado, executa uma ação; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{TextField (Campo de Texto):} Representa um componente que permite a entrada de texto; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{TextArea (Área de Texto):} Representa um componente que permite a entrada de texto em múltiplas linhas; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{CheckBox (Caixa de Seleção):} Representa um componente que permite a seleção de uma ou mais opções; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{Choice (ComboBox ou CaixaCombo):} Representa uma janela na tela, onde os componentes são adicionados; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{ScrollBar (Barra de Rolagem):} Representa um componente que permite a rolagem de uma lista de itens; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{Panel (Painel):} Representa um contêiner que agrupa um conjunto de componentes; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{Canvas (Área de Desenho):} Representa um componente que permite a exibição de gráficos; (CLARO; BOSCO; SOBRAL, [s.d.])
	\item \textbf{MenuBar (Barra de Menu):} Representa um componente que permite a criação de menus; (CLARO; BOSCO; SOBRAL, [s.d.])
\end{itemize}
